				...
				bcrypt.genSalt(10, (err, salt) => {
					bcrypt.hash(newUser.password, salt, (err, hash) => {
						...
						newUser
							...
							.catch(err => console.log('error while saving the user to database' + err));
					});
				});
			}
		})
				...
				bcrypt
					...
					.then(isCorrect => {
						if (isCorrect) {
							...
							return res.status(400).json({ passwordErr: 'Password is not correct' });
						}
					})
					.catch(err => console.log(err));
			}
...
router.get('/products/:domain', (req, res) => {
	Product.find({ domain: req.params.domain })
		...
		.catch(err => console.log(`Error finding products of domain ${req.params.domain} ${err}`));
});
